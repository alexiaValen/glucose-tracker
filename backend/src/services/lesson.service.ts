  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO lessons (title, description, client_id, coach_id, status)
       ...
       RETURNING *`,
      [title, description || null, client_id, coachId]
    );
...
// ✅ GET CLIENT LESSONS
export const getClientLessons = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.error("❌ Fetch client lessons failed:", err);
    res.status(500).json({ error: "Failed to fetch lessons", details: err.message });
  }
};
...
// ✅ MARK VIEWED
export const markLessonViewed = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const userId = req.user!.id;
